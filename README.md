...
서가 점검은 사서가 청구기호를 하나씩 눈으로 훑으며 순서를 확인하는 일이다.

- **단순 반복** — 몇백 권을 같은 방식으로 계속 훑는, 집중력이 갈수록 떨어지는 작업이다.
- **부정확함** — 숫자와 한글이 섞인 청구기호를 사람 눈으로 비교하다 보면 오배열을 놓치기 쉽다.
- **시간이 오래 걸린다** — **책장 하나당 평균 7분.** 서가 전체를 훑으려면 이게 몇 배로 쌓인다.

SHELFY는 이 과정을 사진 한 장으로 줄인다. 서가를 촬영하면 청구기호를 왼→오른 순서로
인식하고, 정렬 규칙에 따라 올바른 순서를 계산해 오배열된 책과 이동할 자리를 바로
알려준다.
